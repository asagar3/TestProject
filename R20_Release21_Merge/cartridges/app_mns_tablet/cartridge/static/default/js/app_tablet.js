	...
	return {
		...
		validatorSettings: {
			...
			highlight: function(element, errorClass) {
				if ($(element).attr("name") == "birthday_day" || $(element).attr("name") == "birthday_month" || $(element).attr("name") == "birthday_year"){ 
					...
					$('#birthday_year').parent().next(".successIcon").removeClass("successIcon").addClass("errorIcon");
				} else {
					$(element).addClass( errorClass ); /*Add 'errorclient' class to the element*/
					if($(element).is('select')){
						var selectElem = $(element).parent(); 
	                    $(selectElem).next(".successIcon").removeClass("successIcon").addClass("errorIcon"); /*change from success icon to error icon*/
			...
			unhighlight: function(element, errorClass) {
				...
				if ($(element).attr("name") == "birthday_day" || $(element).attr("name") == "birthday_month" || $(element).attr("name") == "birthday_year"){ 
					...
					if($(element).is('select')){
						var selectElem = $(element).parent(); 
	                    $(selectElem).next(".errorIcon").addClass("successIcon");				
					}else{
					...
					//QC Defect 828 | The focus is not transferred to submit button on click of Enter key. | Naveen Kumar
					$(element).parents(".formfield").find("div.errorclient").empty();/*Remove Backend errror message*/ 
				}
			},
            ...
            errorPlacement: function(error, element){
            	...
            	if (element.attr("type") != "checkbox"){
                    ...
                    if (element.attr("name") == "birthday_day" || element.attr("name") == "birthday_month" || element.attr("name") == "birthday_year"){ 
                        ...
                        var yearSelectElem = $('#birthday_year').parent(); 
                        if(!$(yearSelectElem).next().hasClass('errorIcon')){
                        	//place only one
                        	errorIconVar.insertAfter(yearSelectElem);
                        }
                        jQuery('.klarnaDoB').find('span.labeltext').addClass('errorlabel');
                    } else { 
                    	if($(element).is('select')){
	                        var selectElem = $(element).parent(); 
	                        if($(selectElem).nextAll('.errorclient:visible').length == 0){// to prevent duplicate error messages :fix 4449
	                        	error.insertAfter(selectElem);
	                            errorIconVar.insertAfter(selectElem);
	                        }
	                     }else{
	                    	 if($(element).nextAll('.errorclient:visible').length == 0){// to prevent duplicate error messages :fix 4449
		...
		minicart: {
			...
			//	else if(jQuery(".minicarttotal").height()<48){  // IE9 fix
					//	jQuery('.minicartcontent').css({top: $(window).scrollTop() + 58}); /*820053*/
			//	}
		...
		//		else if(jQuery(".minicarttotal").height()<48){  // IE9 fix
			...
			slide: function() {
				...
				// show the item
				jQuery('.minicartcontent').hide().slideDown('slow');//show("slide", { direction: "up" }, 1000);
			//	clearTimeout(app.minicart.timer);
			//	app.minicart.timer = null;
			...
			add: function(_this, progressImageSrc, postdata, callback)
			{
				...
				var handlerFunc = function(req)	{
					...
					var minibasketsplited = req.split("<br class='delimiter'>"); 
					// replace the content
					
					var container = jQ(_this).parents('.addtocartbar').siblings('.tablet-minicart');
					
					jQuery('#minicart').html(minibasketsplited[0]);
					container.html(minibasketsplited[1]); 
			...
			show: function(options) {
			    ...
			    jQuery('#QuickViewDialog').dialog('open');
			   	...
			   	/* for all dialogue boxes - set to center in both orientations - Shahzad */
			    	var popupW2 = jQuery('#QuickViewDialog').width();	 
			   	jQuery('#QuickViewDialog').parent().css({ "marginLeft":-(popupW2/2)+"px","left":"50%"});
		...
		validator: function() {
			...
			jQuery.validator.addMethod("positivenumber", function(value, element) {
				var that = this;
				if (value == '') return true;				
				return that.optional(element)  ||  /^-?(?:\d+|\d{1,3}(?:,\d{3})+)?$/.test(value) && Number(value) >= 0;
...
jQuery.validator.addMethod("postcode_fr", function(value, element) {
			...
			/*changed as per CR3103 of website - applied on tablet also : shahzad*/ 	
...
//				if(!elemM.hasClass("required")){return true;}
		...
		producttile : {
			...
			selectVariation : function(swatch) {
				...
				var currentImg = jQ(swatch).closest('.inner-wrapper').find(".productimage img");
				...
				swatch.closest('.swatchWrapper').find("a.swatch").removeClass("selected");
			...
			// this function is binded to touch events for touchscreens
			// where we skip the toggleVariation part as there is not mouseover / mouseout events for touchscreens
			
			selectVariationTablet : function(swatch) {
				// get the new and the original image
			...
			toggleVariationThumbnail : function(swatch,morecolorlink) {
				...
				*/
				...
				var selectedSwatch = swatch.closest('.swatchWrapper').find("a.selected");
				...
				if(currentImg.hasClass("temp")) {
					...
					currentImg.replaceWith(newCopy);
				} else {
...
jQ(function(){
	...
	jQ('body').on('mouseover', '.ui-dialog .tooltip',  function(){
		jQ(this).parents('.ui-dialog').css('overflow', 'visible');
	}).on('mouseout', function(){
		jQ(this).parents('.ui-dialog').css('overflow', 'hidden');
	});
});
...
jQuery(document).ready(function(){
	...
	/*$("#dwfrm_billing_paymentMethods_creditCard_number").bind('keydown', function(event) {
		alert('here');
	...
	jQ('.footer_links_0 li:nth-child(2n)').after("<li class='clear'/>");
