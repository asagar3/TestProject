...
function normalized(a) {
    return (a.indexOf(s_prefix) == 0)
}
function normalizeContent(a, b) {
    if (b) {
        b = b.toLowerCase();
        if (b.indexOf("&amp;") >= 0) {
            b = a.repl(b, "&amp;", "&")
...
function fixContentVars(b) {
    if (b.prop18 && b.eVar25 && (b.prop18 == b.eVar25)) {
        b.eVar25 = "D=c18"
    }
    var a = b.prop18;
    if (!a) {
    ...
    if (b.pageName && !normalized(b.pageName)) {
        if (a && a.indexOf(b.pageName) >= 0) {
            b.pageName = a
        } else {
            if (b.prop24 && (b.prop24 == "Product Detail Page")) {
                b.pageName = a + ":" + b.products
            } else {
                if (b.prop24 && (b.prop24 == "Content Page" || b.prop24 == "My Account Page" || b.prop24 == "Search Result Page")) {
                    b.pageName = a + ":" + b.pageName
                } else {
                    if (b.prop24 && (b.prop24 == "Home Page")) {
                        b.pageName = a + ":Home Page"
                    }
                }
            }
        }
        b.channel = normalizeContent(b, b.channel);
        b.prop1 = normalizeContent(b, b.prop1);
        b.prop18 = normalizeContent(b, b.prop18)
    }
    if (b.products && b.products.indexOf("Parent_") >= 0) {
        b.products = b.repl(b.products, "Parent_", "")
    }
    while (b.products && b.products.indexOf("/") >= 0) {
        b.products = b.repl(b.products, "/", "")
    }
}
function s_doPlugins(n) {
    n.prop10 = n.pageName;
    if (n.campaign) {
        n.prop10 += " CMP: " + n.campaign
    }
    n.prop45 = "D=Host";
    fixContentVars(n);
    if (n.prop3) {
        n.prop3 = n.repl(n.prop3, ".0", "")
    }
    if (n.events) {
        n.events = n.repl(n.events, " ", "")
    }
    n.eVar5 = n.getTimeParting("h", "1");
    n.eVar6 = n.getTimeParting("d", "0");
    n.eVar7 = n.getTimeParting("w", "0");
    n.eVar30 = n.getNewRepeat();
    n.channelManager("intid,cid,cmpid,extid", "", "s_cm", "", "s_dl");
    n.campaign = n._campaignID;
    n.eVar8 = n._channel;
    if (!n.eVar21 && n.eVar8) {
        n.eVar21 = n.crossVisitParticipation(n.eVar8, "s_chst", "7", "5", " > ", "purchase", "0")
    }
    if (!n.eVar20 && n.campaign) {
        n.eVar20 = n.crossVisitParticipation(n.campaign, "s_cmst", "7", "5", " > ", "purchase", "0")
    }
    n.eVar46 = n.getQueryParam("kwid");
	n.eVar15 = n.getAndPersistValue(n.getQueryParam('deviceID'),"s_did",30);
	if(!n.eVar15)
		n.eVar15 = "non-store";
	n.eVar53 = n.getAndPersistValue(n.getQueryParam('orderPoint'),"s_ord",30);
	if(!n.eVar53)
		n.eVar53 = "non-store";
	n.eVar54 = n.getAndPersistValue(n.getQueryParam('orderChannel'),"s_och",30);
	if(!n.eVar54)
		n.eVar54 = "non-store";		
    if (!n.eVar1) {
        n.eVar1 = n.getQueryParam("icid")
    }
    if (!n.eVar1) {
        n.eVar1 = n.getQueryParam("intid")
    }
    if (n.eVar2) {
        n.eVar2 = n.eVar2.toLowerCase()
    }
    var j = n.getValOnce(n[n.searchTermVariable], "ev1", 0);
    if (j == "") {
        if (n.events && n.events.indexOf(n.successfulSearchEvent) >= 0) {
            var l = n.split(n.events, ",");
            var k = "";
            for (var c = 0; c < l.length; c++) {
                if (l[c] == n.successfulSearchEvent) {
                    continue
                } else {
                    if (l[c] == n.nullSearchEvent) {
                        continue
                    } else {
                        k += l[c] ? l[c] + "," : l[c]
                    }
    ...
    if (!n.eVar14 && n.campaign && (n.campaign != "n/a")) {
        n.eVar14 = "external campaign"
    }
    if (n.eVar2 && !n.eVar14) {
        n.eVar14 = "internal search"
    }
    if (n.eVar1 && !n.eVar14) {
        n.eVar14 = "internal campaign"
    }
    if (document.referrer && !n.eVar14) {
        var b = n.split(n.linkInternalFilters, ",");
        var m = false;
        var d = n.split(document.referrer, "/");
        d = d[2];
        for (var h in b) {
            if (d.indexOf(b[h]) > -1) {
                m = true
        ...
        if (!m) {
            n.eVar14 = "external non-campaign"
        }
    }
    n.prop9 = n.getAndPersistValue(n.campaign, "s_cmp_pages");
    if (n.channel && !n.eVar18) {
        n.eVar18 = "D=ch"
    }
    if (n.eVar2 && !n.prop2) {
        n.prop2 = "D=v2"
    ...
    if (n.purchaseID && !n.eVar3) {
        n.eVar3 = n.purchaseID
    }
    if (!n.eVar25) {
        n.eVar25 = "Non-Browse"
    }
...
function s_gi(j, k, B) {
    ...
    if (j) {
        ...
        if (f) {
            for (h = 0; h < f.length; h++) {
                A = f[h];
                q = A._c;
                if ((!q || q == "s_c" || q == "s_l") && (A.oun == j || (A.fs && A.sa && A.fs(A.oun, j)))) {
                    if (A.sa) {
                        A.sa(j)
                    }
                    if (q == "s_c") {
    ...
    t.s_an = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    ...
    p = s_d(p);
    if (o > 0) {
        r = parseInt(h = y.substring(o + 5));
        if (r > 3) {
            r = parseFloat(h)
        }
    } else {
        if (d > 0) {
            r = parseFloat(z.substring(d + 10))
        } else {
            r = parseFloat(y)
        }
    }
    if (r < 5 || y.indexOf("Opera") >= 0 || z.indexOf("Opera") >= 0) {
        p = s_ft(p)
    }
    if (!A) {
        A = new Object;
    ...
    A._c = "s_c";
    (new Function("s", "un", "pg", "ss", p))(A, j, k, B);
    return A
};