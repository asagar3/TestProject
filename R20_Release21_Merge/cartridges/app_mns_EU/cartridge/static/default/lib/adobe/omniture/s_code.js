...
var s_account="mseu5dev"
var s_prefix = "dev:";

if(hn.indexOf("marksandspencer.eu")>-1){
	s_account = "mseu5prod";
	s_prefix = "";
}
...
s.linkInternalFilters="javascript:,.marksandspencer.eu,demandware.net"
s.visitorNamespace="marksandspencer"
s.trackingServer="metrics.marksandspencer.eu"
s.trackingServerSecure="smetrics.marksandspencer.eu"
...
// Channel by pattern
s._channelPattern="Affiliates|af_>Business Media|msfb_>EMWA|emwa>Email|em>Food Media|fd>Indexed Tags|a_msf,b_msf>" +
				  "Media|me>Paid Search|ggl_,ovt_,ps_>Product Feed|c_msf,crit_msf,f_ms,k_msf,pg_msf,s_msf>Social Media|facebook,fb,tw>" +
				  "Traffic Partner|tp_>Video|vid_"
...
{
	return (str.indexOf(s_prefix) == 0);
}

function normalizeContent(s,str)
{
	if (str)
	{
		str = str.toLowerCase();
...
function fixContentVars(s) {

	if (s.prop18 && s.eVar25 && (s.prop18 == s.eVar25))
		s.eVar25 = "D=c18";
	
	var concat_page = s.prop18;
	if (!concat_page)
		concat_page = s.channel;
		
	if (s.pageName && !normalized(s.pageName))
	{
		if (concat_page && concat_page.indexOf(s.pageName)>=0)
			s.pageName = concat_page;
		else if (s.prop24 && (s.prop24 == "Product Detail Page"))
			s.pageName = concat_page + ":"+s.products; 
		else if (s.prop24 && (s.prop24 == "Content Page" || s.prop24 == "My Account Page" || s.prop24 == "Search Result Page"))
			s.pageName = concat_page+":"+ s.pageName;
		else if (s.prop24 && (s.prop24 == "Home Page"))
			s.pageName = concat_page + ":Home Page";
		
		//s.pageName = normalizeContent(s,s.pageName);
		s.channel = normalizeContent(s,s.channel);
		s.prop1 = normalizeContent(s,s.prop1);
		s.prop18 = normalizeContent(s,s.prop18);
	}
	
	if (s.products && s.products.indexOf("Parent_")>=0) 
		s.products = s.repl(s.products,"Parent_","");
	
	//Praveen:- Adding custom code to remove "/" from tNumber.
	while (s.products && s.products.indexOf("/")>=0){
		s.products = s.repl(s.products,"/","");
	}

	// Remove "/" from pageName
	while (s.pageName && s.pageName.indexOf("/")>=0){
...
function s_doPlugins(s) {
	...
	s.prop10=s.pageName;
	if(s.campaign){
	s.prop10+=' CMP: '+s.campaign;
	}
	
	// Collection server tracking
	s.prop45 = 'D=Host';
		
	// !DISABLED - Fix products and pageName tagging issues.	
	fixContentVars(s);
	
	// Change decimal to integer
	if (s.prop3)
		s.prop3 = s.repl(s.prop3,".0","");
		
	// Remove spaces from s.events
	if (s.events)
		s.events = s.repl(s.events," ","");
	
	/* Add calls to plugins here */
	s.eVar5 = s.getTimeParting('h', '1'); 
	s.eVar6 = s.getTimeParting('d', '0');
	s.eVar7 = s.getTimeParting('w', '0');
	
	s.eVar30 = s.getNewRepeat();

	s.channelManager('cid,cmpid,extid','','s_cm','','s_dl');
	s.campaign=s._campaignID;
	s.eVar8=s._channel;
	
	if(!s.eVar21 && s.eVar8)s.eVar21=s.crossVisitParticipation(s.eVar8,'s_chst','7','5',' > ','purchase','0');
	if(!s.eVar20 && s.campaign)s.eVar20=s.crossVisitParticipation(s.campaign,'s_cmst','7','5',' > ','purchase','0');
	
	s.eVar46 = s.getQueryParam('kwid')
	s.eVar15 = s.getAndPersistValue(s.getQueryParam('storeDeviceIds'),"s_sdid",30);
	
	// Instore Tracking //
	/* Device ID */
	s.eVar15 = s.getAndPersistValue(s.getQueryParam('deviceID'),"s_did",30);
	if(!s.eVar15)
		s.eVar15 = "non-store";
		
	/* Order Point */
	s.eVar53 = s.getAndPersistValue(s.getQueryParam('orderPoint'),"s_ord",30);
	if(!s.eVar53)
		s.eVar53 = "non-store";
				
	/* Order Channel */
		s.eVar54 = s.getAndPersistValue(s.getQueryParam('orderChannel'),"s_och",30);
	if(!s.eVar54)
		s.eVar54 = "non-store";		
	...
	if(s.eVar2) s.eVar2=s.eVar2.toLowerCase()

	var t_search=s.getValOnce(s[s.searchTermVariable],'ev1',0)
	if(t_search=='')
	{	
		if (s.events && s.events.indexOf(s.successfulSearchEvent)>=0)
		{
	...
	if(!s.eVar14&&s.campaign&&(s.campaign!='n/a'))
		s.eVar14='external campaign';
	if(s.eVar2&&!s.eVar14)
		s.eVar14='internal search';
	if(s.eVar1&&!s.eVar14)
		s.eVar14='internal campaign';

	/*Other External Sites*/
	if(document.referrer&&!s.eVar14)
	{
		...
		if(!internalFlag)
			s.eVar14="external non-campaign";
	}
	
	s.prop9=s.getAndPersistValue(s.campaign,"s_cmp_pages");
		
	/* Copy duplicate variables */
	if(s.channel&&!s.eVar18) s.eVar18="D=ch";
	if(s.eVar2&&!s.prop2) s.prop2="D=v2";
	if(s.eVar2&&!s.eVar39) s.eVar39='+1';
	if(s.purchaseID&&!s.eVar3) s.eVar3=s.purchaseID;
	if(!s.eVar25) s.eVar25='Non-Browse';
	if(!s.prop8 && s.eVar15) s.prop8='D=v15';
	if (s.eVar5 && !s.prop5) s.prop5 = 'D=v5';
	if (s.eVar6 && !s.prop6) s.prop6 = 'D=v6';
	if (s.eVar7 && !s.prop7) s.prop7 = 'D=v7';
	if (!s.eVar55) s.eVar55 = 'D=s_vi';
	if (!s.prop55) s.prop55 = 'D=s_vi';
}
