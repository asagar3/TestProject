...
function sjFixPNG (img) {
    ...
    if ((version >= 5.5) && (version <=8) && (document.body.filters) && (img.src.indexOf("images/spacer.gif") == -1)){
		var src = img.src;
...
/////////////////////////////////////////////////////
	...
	var zImageURL = "";
		zImageURL = bigImage + sjPBreak(bigImage) + "wid="+Math.round(zoomFactor*smallImage.clientWidth)+
							"&hei="+Math.round(zoomFactor*smallImage.clientHeight)+"&fit=crop,1";
...
function sjFlyout(inId,inURL){
	...
	smallImage.src =  inURL + sjPBreak(inURL) +"wid="+parseInt(getDim(smallImage)[0])+"&hei="+parseInt(getDim(smallImage)[1])+"&fit=crop,1";
	...
	if (document.all) {
		for(var i in document.all){
			for(var j=0; j<names.length;j++){
				if (document.all[i] && document.all[i]["on"+names[j]]){
					document.all[i]["on"+names[j]] = null;
				}
			}
		}
		...
		for(var i in all){
			for(var j=0; j<names.length;j++){
				if (all[i] && all[i]["on"+names[j]]){
					all[i]["on"+names[j]] = null;
				}
			}
		}
	}
